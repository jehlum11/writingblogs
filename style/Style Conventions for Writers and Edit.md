...
Always use a hyphen if clarity would suffer otherwise. For example, "He recovered his health" versus "He re-covered the leaky roof." ⁠[\[1\]](#ftn.id2611)
...
*  Example :  In /some/path/, grep for XYZ. 
  *  Improvement :  In the /some/path/ directory, use the grep command to search for "XYZ". 
*  Example :  param-2 contains a reference to the hostname return value from instance-2. 
  *  Improvement :  The param-2 parameter contains a reference to the hostname return value from your second server instance, instance-2. 
*  Example :  Troubleshoot Glance. 
  *  Improvement :  Troubleshoot the Glance image service. 
*  Example :  Install the maven-changelog-plugin. 
  *  Improvement :  Install the maven-changelog-plugin package. 
...
An entity is a primitive data type, which associates a string with either a unique alias (such as a user-specified name) or an SGML reserved word (such as #DEFAULT) ⁠[\[4\]](#ftn.id4453). Entities are called by reference, and take the form &name; (both the ampersand and the semicolon are required).
...
*   [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**. IBM Corporation, latest version.
    
    Red Hat employees can access the latest version of _IBM Style_ online, at [https://www.ibm.com/docs/en/ibm-style/](https://www.ibm.com/docs/en/ibm-style/). _IBM Style_ does not have a wider circulation. Links in this guide to _IBM Style_ are denoted by a padlock icon  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
_adj._ A less common way to refer to bimodal or hybrid IT. Refer to [bimodal IT](#bimodal-it).
...
_n._ A 64-bit version of the x86 architecture. This term is a synonym of x86\_64. Use this format in general cases to refer to names of the architecture for various cloud providers.
...
_n., adj._ Always use lowercase. Do not capitalize except at the beginning of a sentence, or if it is part of a Red Hat product, service, solution, or business unit name. Refer also to [cloud](#cloud). Big data is also never hyphenated, even when used as a compound modifier.
...
According to [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**, it is acceptable to use "boolean" (lowercase) in API programming information when it refers to a primitive return type.
...
For initial guidance on how to use this term, refer to [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
For more information, refer to the Word Usage section of [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
Correct. Ceph is a distributed storage platform that provides object, block, and file storage. ⁠[\[5\]](#ftn.id5050) Do not use alternative capitalization.
...
Refers to the event where a private cloud exceeds its capacity and "bursts" into and uses public cloud resources. The advantage of such a hybrid cloud deployment is that an organization pays only for extra computing resources when they are needed. ⁠[\[6\]](#ftn.id5114)
...
Do not use as an abbreviation for "combination box". For further usage information, refer to the relevant entry in [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
For more details about how to use these terms, refer to the appropriate entries in [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
_n._ Write as shown, two words, unless used as a variable. For more information, refer to [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
A fully qualified domain name consists of a list of domain labels representing the hierarchy from the lowest relevant level in the DNS to the top-level domain (TLD). The domain labels are concatenated by using the dot or period character (.) as a separator between labels. ⁠[\[7\]](#ftn.id5633)
...
Correct only when referring to fuzzy searches. Refer to [Chapter 4, _Choosing Appropriate Language_](#appropriate-language) for details and examples.
...
_n._ Usually one word. Capitalize the "H" at the beginning of a sentence. For more information, refer to [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
_n._ Always lowercase except in some specific exceptions, for example [Internet of Things (IoT)](#iot).

⁠Internet of Things (IoT)

Correct. Capitalize as shown; spell out on the first occurrence, and use the initialism thereafter.

The Internet of Things (IoT) refers to uniquely identifiable objects and their virtual representations in an internet-like structure. ⁠[\[8\]](#ftn.id6035)

⁠Intranet, intranet

For guidance, refer to the Word Usage section of [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
For the correct abbreviation to use for specific use cases, refer to [_Units of Measurement_](https://www.ibm.com/docs/en/ibm-style?topic=measurement-units) in _IBM Style_  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
_adj._ A network installation system for some Linux distributions. ⁠[\[9\]](#ftn.id6197)

⁠kill

If terminating a UNIX process, use "kill". For example, to terminate the process, type `kill _<PID>_`. If terminating a Windows process, use "terminate". For example, "To terminate the process, press **Q**."
...
_n._ Always lowercase. This is a concept, not a technology or product name. Being a common noun, it normally requires an article. Refer also to [undercloud](#undercloud).
...
* Original text: This [product or solution] is secure.
  * Improvement: This [product or solution] enables you to [do X, Y, and Z] to improve security, by ...
* Original text: This [product or solution] is more secure.
  * Improvement: This [product or solution] is more secure through [describe the steps that are taken]; or This [product or solution] is more secure than [previous state, or competitor example with cited source].
...
Refer to the Word Usage section of [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
_n._ Always lowercase. It is a concept, not a technology or product name. Being a common noun, it normally requires an article. Refer also to [overcloud](#overcloud).
...
_v._ Do not use. Refer to the Word Usage section of [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
...
_n._ Usually one word. Capitalize the "U" at the beginning of a sentence. For more information, refer to [_IBM Style_](https://www.ibm.com/docs/en/ibm-style)  **![](https://www.stylepedia.net/style/images/padlock.png)**.
